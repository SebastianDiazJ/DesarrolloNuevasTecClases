...
const agregarHabilidad = (jugador, nuevaHabilidad) => {
  ...
  //aqui se verifica si la nueva habilidad ya existe en el arreglo de habilidades
  if (!jugador.habilidades.includes(nuevaHabilidad)) {
      jugador.habilidades.push(nuevaHabilidad); // Si no existe,se agrega
      }
...
const buscarPeliculas = (peliculas, titulo, genero) => {
  const peliculasBusqueda = peliculas.filter(pelicula => {
...
const calcularPromedioDuracionPorGenero = (peliculas, genero) => {
    ...
    const promedio = duracionTotal / peliculasDelGenero.length;
    return promedio;
...
class Bicicleta extends Vehiculo {
   ...
   */
  obtenerInformacion() {
      return `Marca: ${this.marca}, Modelo: ${this.modelo}, Año: ${this.año}, Tipo: ${this.tipo}, Marchas: ${this.numMarchas}, Material: ${this.material}, Suspensión: ${this.suspencion}, Frenos: ${this.frenos}, Manubrio: ${this.tipoManubrio}`;
  }
}
